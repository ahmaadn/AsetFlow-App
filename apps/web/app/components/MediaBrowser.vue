...
const selectedAssetTypeFilter = ref<ComboBoxOption | null>(
  assetTypeOptions.find((o) => o.value === props.selectedAssetType) || null
);

// Watch asset type filter changes
watch(selectedAssetTypeFilter, (newVal) => {
  const type = (newVal?.value as AssetType) || 'all';
  emit('asset-type-change', type);
});
  ...
  <div class="flex-1 relative">
    ...
    <div class="space-y-4">
      ...
      >
        <template #left>
          <!-- Back Button -->
          <button class="btn bg-base-100 btn-sm gap-2" @click="emit('back')">
            <Icon name="ri:arrow-left-line" class="size-4" />
            Back
          </button>

          <!-- Refresh Button -->
          <button class="btn bg-base-100 btn-sm gap-2" @click="emit('refresh')">
